...
import { JourneyContents } from './About/Contents';
...
const CircleElement: React.FC<CircleElementProps> = ({ index, year,  description, imageUrl, isLeftAlign }) => {
    ...
    useEffect(() => {
        if (!containerRef.current || !circleRef.current || !contentRef.current || !imageRef.current) return;
        ...
        >
            ...
            {isLeftAlign ? (
                <>
                    ...
                    <div ref={imageRef} className="absolute right-full flex items-center mr-4">
                        <div className="w-[300px] h-[100px] relative overflow-hidden rounded-lg shadow-md">
                            <Image 
                                src={imageUrl} 
                    ...
                    <div ref={contentRef} className="absolute left-full flex items-center ml-4">
                        <div className="text-left w-[200px]">
                            <div className="text-lg font-bold text-[#005F20]">{year}</div>
                ...
                <>
                    ...
                    <div ref={contentRef} className="absolute right-full flex items-center mr-4">
                    ...
                    <div ref={imageRef} className="absolute left-full flex items-center ml-4">
                        <div className="w-[300px] h-[100px] relative overflow-hidden rounded-lg shadow-md">
                            <Image 
                                src={imageUrl} 
                                alt={`Journey milestone for ${year}`}
...
const ScrollProgressLine: React.FC = () => {
    ...
    return (
        <div className="min-h-screen max-w-full py-30">
            {/* Alternating pattern container */}
            <div className="w-full max-w-[150px] mx-auto px-8">
                {elements.map((element, index) => (
                    <div key={index} className='w-full'>
                        {element.type === 'circle' ? (
                            <CircleElement 
                                index={index} 
                                year={JourneyContents[Math.floor(index / 2)]?.year || "2025"}
                                description={JourneyContents[Math.floor(index / 2)]?.content || "Description"}

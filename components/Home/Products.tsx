...
const Products: React.FC = () => {
  ...
  // Desktop card component without animation
  ...
  const MobileCard: React.FC<{ product: Product; index: number }> = ({ product, index }) => {
        ...
        <div className="bg-white rounded-lg   transition-shadow duration-300 overflow-hidden group">
          <div className="relative h-64 overflow-hidden md:h-[369px] h-[212px]">
            <Image
              ...
              alt="Product Image"
              fill
              className="object-cover group-hover:scale-105 transition-transform duration-300 md:h-[369px] h-[212px]"
            />
          </div>
        </div>
      );
    ...
    return (
      <div
        style={{ backgroundColor: product.bg }}
        className="rounded-lg border border-[#CFCFCF] transition-shadow duration-300 overflow-hidden group md:h-[369px] h-[212px]"
      >
        <div className="block h-full md:p-8 px-2 py-5">
          <div className="flex flex-col h-full justify-between">
            <h3
              style={{ color: product.text }}
            ...
            <div className="flex flex-col items-start">
              <p
                style={{ color: product.text }}
                className="text-[#4A4A4A] md:text-[14px] text-[10px] mb-1"
              >
                {product.description}
              </p>
              <Link
  ...
  return (
    <div className=" md:py-20 py-10 px-10 bg-[#F6F6F6]">
      ...
      <motion.div>
        ...
        {/* Mobile Layout */}
        <div className="md:hidden container mx-auto px-1 md:mt-20 py-8">
          ...
          {productsContent.length > 4 && (
            <div className="flex justify-center mt-8">
              <button
                ...
                className="text-[#191919] text-md font-medium transition-colors duration-300 flex items-center gap-2"
              >
